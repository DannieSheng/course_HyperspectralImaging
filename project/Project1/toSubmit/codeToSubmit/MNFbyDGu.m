function  [YVecs, transform] = MNFbyDGu(X, Parameters)
% function  [YVecs, Cn, ReconX, W] = MNFbyDGu(X, Parameters)
% An updated version of MNFbyDF function, which take the input X of a
% different size
%
%%% X is an N X B Spectral Data 
%%%     where
%%%        B = Number of Bands
...
for b = 1:B
    XSlice           = squeeze(X(:, b));
    SubNbrs(:, b) = conv(XSlice, NoiseMask, 'same');
end
