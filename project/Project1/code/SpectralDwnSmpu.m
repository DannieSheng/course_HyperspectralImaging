 function [DsX, DsWvs, transform] = SpectralDwnSmpu(X, Wvs, DwnSmpRate, varargin)
% An updated version of function SpectralDwnSmpu, which take the input X of a
% different size
%
...
%         ConvMask = ones(1,1,DwnSmpRate)/DwnSmpRate;
        ConvMask = ones(1,DwnSmpRate)/DwnSmpRate;
    end
    
    %%% CALCULATE CONVOLUTION %%%
    DsX = conv2(X, ConvMask, 'same');
    ...
    transform.DwnSmpRate = DwnSmpRate;
    transform.ConvMask   = ConvMask;
else
